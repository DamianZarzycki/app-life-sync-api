...
export const ListNotesQuerySchema = z.object({
  ...
  include_deleted: z
    .enum(['true', 'false'], { errorMap: () => ({ message: 'include_deleted must be true or false' }) })
  ...
  sort: z
    .enum(['created_at_desc', 'created_at_asc', 'updated_at_desc'], {
      errorMap: () => ({ message: 'sort must be one of: created_at_desc, created_at_asc, updated_at_desc' }),
    })
    .optional()
...
export const CreateNoteCommandSchema = z.object({
  ...
  title: z
    ...
    .default(null)
    .refine(
      (val) => val === null || (typeof val === 'string' && val.length <= 255),
